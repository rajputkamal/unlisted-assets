...
import DematDetails from "../DematDetails/Index"
import PGOrder from "../PGOrder/Index"
...
import VirtualAccountBookManager from '../VirtualAccountBookManager/Index';
...
import ShareTransfer from '../ShareTransfer/Index'
import PaymentIssueResolution from '../PaymentIssueResolution/Index'
import { clearAccessToken, apiCall } from "../../../Utils/Network"
import { useHistory } from "react-router-dom";
import MisHolding from '../MisHolding/MisHolding';
import RoleUserView from '../RoleUserView/Index';
import CustomBuyRequest from '../CustomBuyRequest/Index';
import CustomSellRequest from '../CustomSellRequest/Index';
    ...
    <div className={classes.root}>
      <div className="row mb-4">
        <div className="col-md-3 ">
          <div className="my-card user_role_tabs pl-0 pr-0" style={{height: '100vh'}}>
            ...
            >
              <div className="text-left pl-3 pt-2 border-bottom">
                <h6 className="text-default"><b>User Baikhata and Security</b></h6>
              </div>

              {myRoles.includes("User_Role_Mapping_Allowed") ?
                <Tab label="User-Role Mapping Admin" value={0}
                  onClick={(e) => handleChange(0)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("User_Role_Mapping_Allowed") ?
                <Tab label="Role User View" value={30}
                  onClick={(e) => handleChange(30)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Role_Permission_Mapping_Allowed") ?
                <Tab label="Role-Permission Mapping Admin" value={1}
                  onClick={(e) => handleChange(1)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("User_Manager_Allowed") ?
                <Tab label="Users Manager" value={2}
                  onClick={(e) => handleChange(2)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Role_Manager_Allowed") ?
                <Tab label="Role Manager" value={3}
                  onClick={(e) => handleChange(3)} className="user-dashboard-Tablink" />
                : <Tab label="Role Manager" value={3}
                  onClick={(e) => handleChange(3)} className="user-dashboard-Tablink" />
              }

              {myRoles.includes("Permission_Manager_Allowed") ?
                <Tab label="Permission Manager" value={4}
                  onClick={(e) => handleChange(4)} className="user-dashboard-Tablink" />
                : null}


              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>User Journey</b></h6>
              </div>

              {myRoles.includes("Onboarding_Manager_Allowed") ?
                <Tab label="Onboarding Manager" value={5}
                  onClick={(e) => handleChange(5)} className="user-dashboard-Tablink" />
                : null}
              {myRoles.includes("Onboarding_Manager_Allowed") ?
                <Tab label="Bank Details" value={31}
                  onClick={(e) => handleChange(31)} className="user-dashboard-Tablink" />
                : null}
              {myRoles.includes("Onboarding_Manager_Allowed") ?
                <Tab label="Demat Details" value={32}
                  onClick={(e) => handleChange(32)} className="user-dashboard-Tablink" />
                : null}
              {myRoles.includes("Holding_Manager_Allowed") ?
                <Tab label="Holding Manager" value={6}
                  onClick={(e) => handleChange(6)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Listing_Manager_Allowed") ?
                <Tab label="Listing Manager" value={7}
                  onClick={(e) => handleChange(7)} className="user-dashboard-Tablink" />
                : null}

              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Company Care Taker</b></h6>
              </div>

              {myRoles.includes("Company_Manager_Allowed") ?
                <Tab label="Company Manager" value={8}
                  onClick={(e) => handleChange(8)} className="user-dashboard-Tablink" />
                : null}
              {myRoles.includes("Sector_Manager_Allowed") ?
                <Tab label="Sector Manager" value={9}
                  onClick={(e) => handleChange(9)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Funding_Manager_Allowed") ?
                <Tab label="Funding Series Manager" value={10}
                  onClick={(e) => handleChange(10)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Company_Type_Manager_Allowed") ?
                <Tab label="Company Type Manager" value={11}
                  onClick={(e) => handleChange(11)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Add_Company_Request_Manager_Allowed") ?
                <Tab label="Add Company Request Manager" value={12}
                  onClick={(e) => handleChange(12)} className="user-dashboard-Tablink" />
                : null}

              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Auditors - Internal, External</b></h6>
              </div>

              {myRoles.includes("Auditors_External_Admin_Allowed") ?
                <Tab label="Auditors External Admin" value={13}
                  onClick={(e) => handleChange(13)} className="user-dashboard-Tablink" />
                : null}
              {/*<Tab label="Auditors External Admin" {...a11yProps(13)} className="user-dashboard-Tablink"/>*/}

              {myRoles.includes("Auditors_Internal_Admin_Allowed") ?
                <Tab label="Auditors Internal Admin" value={14}
                  onClick={(e) => handleChange(14)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Auditor_Admin_Allowed") ?
                <Tab label="Auditor Admin Manager" value={15}
                  onClick={(e) => handleChange(15)} className="user-dashboard-Tablink" />
                : null}

              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Global Configurations</b></h6>
              </div>

              {/* {myRoles.includes("Module_Manager_Allowed") ?
                <Tab label="Module Manager" value={16}
                  onClick={(e) => handleChange(16)} className="user-dashboard-Tablink" />
                : null} */}

              {myRoles.includes("Decision_Grid_Manager_Allowed") ?
                <Tab label="Decision Grid Manager" value={17}
                  onClick={(e) => handleChange(17)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Template_Manager_Allowed") ?
                <Tab label="Template Manager" value={18}
                  onClick={(e) => handleChange(18)} className="user-dashboard-Tablink" />
                : null}

              {/*<Tab label="Feature Manager" {...a11yProps(22)} className="user-dashboard-Tablink"/>*/}

              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Marketplace Maker</b></h6>
              </div>

              {myRoles.includes("Global_News_Manager_Allowed") ?
                <Tab label="Global News Manager" value={19}
                  onClick={(e) => handleChange(19)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Global_Help_Manager_Allowed") ?
                <Tab label="Global Help Manager" value={20}
                  onClick={(e) => handleChange(20)} className="user-dashboard-Tablink" />
                : null}


              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Notification Hero</b></h6>
              </div>

              {myRoles.includes("Notification_User_Category_Manager_Allowed") ?
                <Tab label="Notification User Category Manager" value={21}
                  onClick={(e) => handleChange(21)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Notification_User_Manager_Allowed") ?
                <Tab label="Notification User Manager" value={22}
                  onClick={(e) => handleChange(22)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Notification_Schedular_Manager_Allowed") ?
                <Tab label="Notification Schedular Manager" value={23}
                  onClick={(e) => handleChange(23)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Negotiation_Comment_Manager_Allowed") ?
                <Tab label="Negotiation Comment Manager" value={24}
                  onClick={(e) => handleChange(24)} className="user-dashboard-Tablink" />
                : null}

              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Services</b></h6>
              </div>

              {myRoles.includes("Loan_Against_Shares_Manager_Allowed") ?
                <Tab label="Loan Against Shares Manager" value={25}
                  onClick={(e) => handleChange(25)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Valuation_Shares_Manager_Allowed") ?
                <Tab label="Valuation Shares Manager" value={26}
                  onClick={(e) => handleChange(26)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Find_Broker_Manager_Allowed") ?
                <Tab label="Find Broker Manager" value={27}
                  onClick={(e) => handleChange(27)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="Custom Buy Request" value={34}
                  onClick={(e) => handleChange(34)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="Custom Sell Request" value={35}
                  onClick={(e) => handleChange(35)} className="user-dashboard-Tablink" />
                : null}

              <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Virtual Account</b></h6>
              </div>

              {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="Money Transfer" value={28}
                  onClick={(e) => handleChange(28)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="PG Order" value={33}
                  onClick={(e) => handleChange(33)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="Mis Holding" value={29}
                  onClick={(e) => handleChange(29)} className="user-dashboard-Tablink" />
                : null}

              {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="Virtual Account Book Manager" value={38}
                  onClick={(e) => handleChange(38)} className="user-dashboard-Tablink" />
                : null}

              {/* <div className="text-left pl-3 pt-2 border-bottom border-top">
                <h6 className="text-default"><b>Payment Issue</b></h6>
              </div> */}

{/* 
              {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="Payment Issue Resolution" value={37}
                  onClick={(e) => handleChange(37)} className="user-dashboard-Tablink" />
                : null} */}


              {/* {myRoles.includes("Money_Transfer_Allowed") ?
                <Tab label="Share Transfer" value={36}
                  onClick={(e) => handleChange(36)} className="user-dashboard-Tablink" />
                : null} */}
