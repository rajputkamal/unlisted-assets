...
export default function InventoryTableContent(props) {
    ...
    const getSearchOption = async function () {
        let response1 = await apiCall("company/allfundingseriesnames", 'GET', '', history)
    ...
    const getAllInventoryget = async function () {
        let response1 = await apiCall("fundingSeries", 'GET', '', history)
        // console.log(response1)
        if (response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        ...
        let responseJSON1 = await response1.json();
        // console.log(responseJSON1._embedded.fundingSeries)
        setRowUser(responseJSON1._embedded.fundingSeries)
        setLoad(true);
    }

    const getAllInventoryupdate = async function (id) {
        let response1 = await apiCall("fundingSeries/" + editedData.id, 'PUT', editedData, history)
        // console.log(response1)
        if (response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
    ...
    const getAllInventorydelete = async function (id) {
        let response1 = await apiCall("fundingSeries/" + editedData.id, 'DELETE', '', history)
        // console.log(response1)
        if (response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
    ...
    const getAllInventoryadd = async function () {
        let response1 = await apiCall("fundingSeries", 'POST', editedData, history)
        // console.log(response1)
        if (response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
            ...
            <div className="dbmn"> 
                <div className="Table_title">
                    <div className="d-flex align-items-center justify-content-start w-100">
                        <h5 className='main-heading'><b>Funding Series Manager</b></h5>
                    </div>
                </div>
                                ...
                                rowUser.map(
                                    (user, index) => (
                                        <TableRow key={index} id={user.id}>
                                            ...
                                            <TableCell>{isload ? <> {user.label} </> : <Skeleton width={100} />}</TableCell>
                                            <TableCell>{isload ? <> {user.value} </> : <Skeleton width={100} />}</TableCell>
                                            <TableCell >
                        ...
                        <div className="modal-body">
                            <form className="w-100">
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">Label -</label>
                                    <input type="text" value={editedData.label} onChange={(e) => handleChange("label", e.target.value)}
                                        className="form-control" />
                                </div>
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">Value -</label>
                                    <input type="text" value={editedData.value} onChange={(e) => handleChange("value", e.target.value)}
                                        className="form-control" />
                                </div>
