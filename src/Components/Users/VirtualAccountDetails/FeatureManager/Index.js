...
import { apiCall, apiCall1, apiCall12, apiCall25, downloadurl, setAccessToken } from "../../../../Utils/Network"
...
export default function InventoryTableContent(props){
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall12("features/search/findByModuleId?id="+props.company.id,'GET', '', history)
    ...
    const getAllInventoryupdate = async function (id){
        let response1 = await apiCall("features/"+editedData.id,'PUT', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
