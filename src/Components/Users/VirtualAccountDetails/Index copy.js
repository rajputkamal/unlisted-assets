...
export default function InventoryTableContent(props){  
  ...
  // }, []);
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall("modules",'GET', '', history)
        if(response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        }
        // console.log(response1)
        console.log("response1 modules get all data", response1)
        let responseJSON1 = await response1.json();
        console.log("responseJSON1 modules all data", responseJSON1._embedded.modules)
        // setRowUser(responseJSON1._embedded.modules)
        // setLoad(true);
    }

    // for virtual account 

    const getAllInventoryget2 = async function (){
    ...
    const getAllInventoryupdate = async function (id){
        let response1 = await apiCall("modules/"+editedData.id,'PUT', editedData, history)
        if(response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        }
    ...
    const getAllInventorydelete = async function (id){
        let response1 = await apiCall("modules/"+editedData.id,'DELETE', '', history)
        if(response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        }
                            ...
                            <div className="modal-body">
                                <form className="w-100">
                                    <div className="form-control-row">
                                        ...
                                        <input type="text" value={editedData.withDrawal} onChange={(e) => handleChange("withDrawal", e.target.value)}
                                    ...
                                    <div className="form-control-row">
                                        ...
                                        <input type="text" value={editedData.transfered} onChange={(e) => handleChange("deposit", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">Balance -</label>
                                        <input type="text" value={editedData.balance} onChange={(e) => handleChange("deposit", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">VirtualBankDetail</label>
                                        <input type="text" value={editedData.virtualBankDetail} onChange={(e) => handleChange("deposit", e.target.value)}
                                               className="form-control" />
                                    </div>
                    ...
                    <Fade in={openAddModal}>
                        <div className={classes.paper}>
                            ...
                            <div className="modal-body pb-5">
                                <form className="w-100">
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">date -</label>
                                        <input type="text" value={newElement.updateDate} onChange={(e) => setNewElement({...newElement, updateDate:e.target.value})}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">ac -</label>
                                        <input type="text" value={newElement.virtualBankDetail} onChange={(e) => setNewElement({...newElement, virtualBankDetail:e.target.value})}
                                               className="form-control" />
                                    </div>
                                </form>
                            </div>
                            <div className="modal-footer p-1">
                                <div className="d-flex align-items-center">
                                    ...
                                    {!isLoadingbtn && (
                                        <Buttons.PrimaryButton value="Submit" style={{width:"100%", margin:"0px 3px"}} onClick={addNewDatainTable}/>
                                    )}
                            ...
                            <div className="modal-footer p-1">
                                <div className="d-flex align-items-center">
                                    <Buttons.SecondaryButton onClick={handleCloseDeleteModal} value="Cancel" style={{width:"100%", margin:"0px 3px"}}/>
                                     {!isLoadingbtn && (
                                        <Buttons.PrimaryButton value="Confirm" style={{width:"100%", margin:"0px 3px"}} 
