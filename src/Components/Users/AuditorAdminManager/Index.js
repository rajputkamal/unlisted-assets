...
export default function InventoryTableContent(props) {
    ...
    const getAllInventoryget = async function () {
        let response1 = await apiCall("auditorAdmins", 'GET', '', history)
        ...
        // console.log(responseJSON1._embedded.auditorAdmins)
        setRowUser(responseJSON1._embedded.auditorAdmins)
        setLoad(true);
    }

    const getAllModuleNames = async function () {

        let response2 = await apiCall("modules", 'GET', '', history)
        if (response2.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        }
        let responseJSON2 = await response2.json();
        let responseJSON21 = responseJSON2._embedded.modules
        // console.log(responseJSON21)
        let modules = responseJSON21.map((module) => { return { value: module.id, label: module.name } })
        await setmodules(modules)
    }

    const getFeaturesForModule = async (moduleId) => {
        setselectedFeature('')
        let response1 = await apiCall12("features/search/findByModuleId?id=" + moduleId, 'GET', '', history)
        if (response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        }
        ...
        let features = responseJSON1._embedded.features.map((feature) => { return { value: feature.id, label: feature.name } })
        await setFeatures(features)
    }

    const getAllInventoryupdate = async function (id) {
        let response1 = await apiCall("auditorAdmins/" + editedData.id, 'PUT', editedData, history)
        // console.log(response1)
        // let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id) {
        let response1 = await apiCall("auditorAdmins/" + editedData.id, 'DELETE', '', history)
        if (response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        }
    ...
    const getAllInventoryadd = async function () {
        ...
        let response1 = await apiCall("auditorAdmins", 'POST', editedData, history)
        if (response1.status == undefined) {
            // errorToast("Invalid", "Invalid User ID or password");
            return
        }
    ...
    const handleOpenEditModal = async (userobject) => {
        setselectedFeature('')
        setEditedData(userobject)
        setOpenEditModal(true);
        let module = modules.filter(module => module.label == userobject.screenName)
        setselectedModule(module[0])
        //console.log("qqqqqqq"+module[0].value+userobject.screenName)
        await getFeaturesForModule(module[0].value)
        let feature = features.filter(feature => feature.label == userobject.type)
        await setselectedFeature(feature[0])
    };
                    ...
                    <div className='col-md-8 col-12'>
                        <div className="Table_title">
                            <div className="d-flex align-items-center justify-content-start w-100">
                                <h5 className='main-heading'><b>Auditor Admin Manager</b></h5>
                            </div>
                        </div>
                    </div>
                                ...
                                rowUser.map(
                                    (user, index) => (
                                        <TableRow key={index} id={user.id}>
                                            ...
                                            <TableCell>{isload ? <> {user.screenName} </> : <Skeleton width={100} />}</TableCell>
                                            ...
                                            <TableCell >
                                                <div className="d-flex justify-content-between">
                                                    {isload ?
                                                        <button className="btn btn-primary-default" onClick={() => handleOpenEditModal(user)}>
                                                            <EditIcon />
                                                        </button>
                                                        :
                        ...
                        <div className="modal-body">
                            <form className="w-100">
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">screenNameOrModuleName -</label>
                                    <Select options={modules} onChange={selectedOption => {
                                        setselectedModule(selectedOption)
                                        handleChange("screenName", selectedOption.label)

                                        getFeaturesForModule(selectedOption.value)
                                    }} value={selectedModule} />

                                </div>
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">type -</label>
                                    <Select options={features} onChange={selectedOption => {
