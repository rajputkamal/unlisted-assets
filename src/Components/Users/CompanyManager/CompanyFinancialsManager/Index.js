...
export default function InventoryTableContent(props){
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall12("companyFinancials1s/search/findByCompanyId?id="+props.company.id,'GET', '', history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1._embedded.companyFinancials1s)
        setRowUser(responseJSON1._embedded.companyFinancials1s)
    }

    const getAllInventoryupdate = async function (id){
        let response1 = await apiCall("companyFinancials1s/"+editedData.id,'PUT', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id){
        let response1 = await apiCall("companyFinancials1s/"+editedData.id,'DELETE', '', history)
        //console.log(response1)
        // let responseJSON1 = await response1.json();
    ...
    const getAllInventoryadd = async function (){
        ...
        let response1 = await apiCall("companyFinancials1s",'POST', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1)
                                ...
                                (user,index) => (
                                  <TableRow  key={index} id={user.id}>
                                    ...
                                    <TableCell>{ isload ? <> {props.company.cin} </> : <Skeleton width={100} /> }</TableCell>
                                      ...
                                      <TableCell>{ isload ? <> {user.fy_2016} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.fy_2017} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.fy_2018} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.fy_2019} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.fy_2020} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.associatedNumber} </> : <Skeleton width={100} /> }</TableCell>
                        ...
                        <div className={classes.paper}>
                            ...
                            <div className="modal-body">
                                <form className="w-100">
                                    ...
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">FY_2016 -</label>
                                        <input type="text" value={editedData.fy_2016} onChange={(e) => handleChange("fy_2016", e.target.value)}
                                               className="form-control" />
                                    </div>

                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">FY_2017 -</label>
                                        <input type="text" value={editedData.fy_2017} onChange={(e) => handleChange("fy_2017", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">FY_2018 -</label>
                                        <input type="text" value={editedData.fy_2018} onChange={(e) => handleChange("fy_2018", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">FY_2019 -</label>
                                        <input type="text" value={editedData.fy_2019} onChange={(e) => handleChange("fy_2019", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">FY_2020 -</label>
                                        <input type="text" value={editedData.fy_2020} onChange={(e) => handleChange("fy_2020", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">associatedNumber -</label>
                                        <input type="text" value={editedData.associatedNumber} onChange={(e) => handleChange("associatedNumber", e.target.value)}
                                               className="form-control" />
                                    </div>
                                </form>
                            </div>
