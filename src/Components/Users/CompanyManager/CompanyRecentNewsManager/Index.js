...
export default function InventoryTableContent(props){
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall12("companyRecentNews/search/findByCompanyId?id="+props.company.id,'GET', '', history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1._embedded.companyRecentNews)
        setRowUser(responseJSON1._embedded.companyRecentNews)
    }

    const getAllInventoryupdate = async function (id){
        let response1 = await apiCall("companyRecentNews/"+editedData.id,'PUT', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id){
        let response1 = await apiCall("companyRecentNews/"+editedData.id,'DELETE', '', history)
        //console.log(response1)
        // let responseJSON1 = await response1.json();
    ...
    const getAllInventoryadd = async function (){
        ...
        let response1 = await apiCall("companyRecentNews",'POST', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1)
                            ...
                            rowUser.map(
                                (user,index) => (
                                  <TableRow  key={index} id={user.id}>
                                    ...
                                    <TableCell>{ isload ? <> {user.title} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.description} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.associatedLink} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.associated_date} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.associatedSource} </> : <Skeleton width={100} /> }</TableCell>
                        ...
                        <div className={classes.paper}>
                            ...
                            <div className="modal-body">
                                <form className="w-100">
                                    ...
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">Image -</label>
                                        <input type="text" value={editedData.image} onChange={(e) => handleChange("image", e.target.value)}
                                               className="form-control" />
                                    </div>

                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">Title -</label>
                                        <input type="text" value={editedData.title} onChange={(e) => handleChange("title", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    ...
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">associatedLink -</label>
                                        <input type="text" value={editedData.associatedLink} onChange={(e) => handleChange("associatedLink", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">associated_date - dd-MM-yyyy</label>
                                        <input type="text" value={editedData.associated_date} onChange={(e) => handleChange("associated_date", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">associatedSource -</label>
                                        <input type="text" value={editedData.associatedSource} onChange={(e) => handleChange("associatedSource", e.target.value)}
                                               className="form-control" />
                                    </div>
                                </form>
                            </div>
                        ...
                        <div className={classes.paper}>
                            ...
                            <div className="modal-body pb-5">
                                <form className="w-100">
                                    ...
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">associated_date -</label>
                                        <input type="text" value={editedData.associated_date} onChange={(e) => handleChange("associated_date", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
