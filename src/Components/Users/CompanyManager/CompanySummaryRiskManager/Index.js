...
export default function InventoryTableContent(props){
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall12("companySummaryRisks/search/findByCompanyId?id="+props.company.id,'GET', '', history)
        // console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1._embedded.companySummaryRisks)
        setRowUser(responseJSON1._embedded.companySummaryRisks)
    }

    const getAllInventoryupdate = async function (id){
        let response1 = await apiCall("companySummaryRisks/"+editedData.id,'PUT', editedData, history)
        // console.log(response1)
        let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id){
        let response1 = await apiCall("companySummaryRisks/"+editedData.id,'DELETE', '', history)
        //console.log(response1)
        // let responseJSON1 = await response1.json();
    ...
    const getAllInventoryadd = async function (){
        ...
        let response1 = await apiCall("companySummaryRisks",'POST', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1)
                                ...
                                (user,index) => (
                                  <TableRow  key={index} id={user.id}>
                                    ...
                                    <TableCell>{ isload ? <> {user.description} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.ordersuggested} </> : <Skeleton width={100} /> }</TableCell>
                                     <TableCell >
                        ...
                        <div className={classes.paper}>
                            ...
                            <div className="modal-body">
                                <form className="w-100">
                                    ...
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">ordersuggested -</label>
                                        <input type="text" value={editedData.ordersuggested} onChange={(e) => handleChange("ordersuggested", e.target.value)}
                                               className="form-control" />
                                    </div>
