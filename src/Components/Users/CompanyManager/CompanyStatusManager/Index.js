...
export default function InventoryTableContent(props){
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall12("companyStatuses/search/findByCompanyId?id="+props.company.id,'GET', '', history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1._embedded.companyStatuses)
        setRowUser(responseJSON1._embedded.companyStatuses)
    }

    const getAllInventoryupdate = async function (id){
        let response1 = await apiCall("companyStatuses/"+editedData.id,'PUT', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id){
        let response1 = await apiCall("companyStatuses/"+editedData.id,'DELETE', '', history)
        //console.log(response1)
        // let responseJSON1 = await response1.json();
    ...
    const getAllInventoryadd = async function (){
        ...
        let response1 = await apiCall("companyStatuses",'POST', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1)
                                ...
                                (user,index) => (
                                  <TableRow  key={index} id={user.id}>
                                    ...
                                    <TableCell>{ isload ? <> {props.company.cin} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.dhrpFilled+""} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.rhpFilled+""} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.ipoDatesAnnounced+""} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.ipoTentativeDate} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.listed+""} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.blackListed+""} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.merged+""} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.split+""} </> : <Skeleton width={100} /> }</TableCell>
                        ...
                        <div className={classes.paper}>
                            ...
                            <div className="modal-body">
                                <form className="w-100">
                                    ...
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">dhrp filled -</label>
                                        <input  id="styled-checkbox-2" type="checkbox" checked={editedData.dhrpFilled}
                                                onChange={(e) => {

                                                    handleChange("dhrpFilled", e.target.checked)
                                                }}/>
                                    </div>

                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">rhp filled -</label>
                                        <input  id="styled-checkbox-2" type="checkbox" checked={editedData.rhpFilled}
                                                onChange={(e) => {
                                                    //console.log("123232323232323232" + e.target.checked)
                                                    handleChange("rhpFilled", e.target.checked)
                                                }}/>
                                    </div>

                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">ipoDatesAnnounced -</label>
                                        <input  id="styled-checkbox-2" type="checkbox" checked={editedData.ipoDatesAnnounced}
                                                onChange={(e) => {

                                                    handleChange("ipoDatesAnnounced", e.target.checked)
                                                }}/>
                                    </div>

                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">IPO tentative date (YYYY-MM-DD) -</label>
                                        <input  id="ipotentativedate" type="text" value={editedData.ipoTentativeDate}
                                                onChange={(e) => {
                                                    //console.log("123232323232323232" + e.target.value)
                                                    handleChange("ipoTentativeDate", e.target.value)
                                                }}/>
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">listed -</label>
                                        <input  id="styled-checkbox-2" type="checkbox" checked={editedData.listed}
                                                onChange={(e) => {

                                                    handleChange("listed", e.target.checked)
                                                }}/>
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">blackListed -</label>
                                        <input  id="styled-checkbox-2" type="checkbox" checked={editedData.blackListed}
                                                onChange={(e) => {
                                                    //console.log("123232323232323232" + e.target.checked)
                                                    handleChange("blackListed", e.target.checked)
                                                }}/>
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">merged -</label>
                                        <input  id="styled-checkbox-2" type="checkbox" checked={editedData.merged}
                                                onChange={(e) => {

                                                    handleChange("merged", e.target.checked)
                                                }}/>
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">split -</label>
                                        <input  id="styled-checkbox-2" type="checkbox" checked={editedData.split}
                                                onChange={(e) => {
                                                    //console.log("123232323232323232" + e.target.checked)
                                                    handleChange("split", e.target.checked)
                                                }}/>
                                    </div>
