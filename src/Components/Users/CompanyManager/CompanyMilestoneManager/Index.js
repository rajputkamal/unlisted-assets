...
export default function InventoryTableContent(props){
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall12("companyMilestones/search/findByCompanyId?id="+props.company.id,'GET', '', history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1._embedded.companyMilestones)
        setRowUser(responseJSON1._embedded.companyMilestones)
    }

    const getAllInventoryupdate = async function (id){
        let response1 = await apiCall("companyMilestones/"+editedData.id,'PUT', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id){
        let response1 = await apiCall("companyMilestones/"+editedData.id,'DELETE', '', history)
        //console.log(response1)
        // let responseJSON1 = await response1.json();
    ...
    const getAllInventoryadd = async function (){
        ...
        let response1 = await apiCall("companyMilestones",'POST', editedData, history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1)
                                ...
                                (user,index) => (
                                  <TableRow  key={index} id={user.id}>
                                    ...
                                    <TableCell>{ isload ? <> {props.company.cin} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.year} </> : <Skeleton width={100} /> }</TableCell>
                                      <TableCell>{ isload ? <> {user.milestone} </> : <Skeleton width={100} /> }</TableCell>
                                     <TableCell >
                        ...
                        <div className={classes.paper}>
                            ...
                            <div className="modal-body">
                                <form className="w-100">
                                    ...
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">Year -</label>
                                        <input type="text" value={editedData.year} onChange={(e) => handleChange("year", e.target.value)}
                                               className="form-control" />
                                    </div>
                                    <div className="form-control-row">
                                        <label className="m-0 mt-2 text-small">Milestone -</label>
                                        <input type="text" value={editedData.milestone} onChange={(e) => handleChange("milestone", e.target.value)}
                                               className="form-control" />
                                    </div>
