...
export default function CustomBuyRequest(props) {
    ...
    const getAllInventoryget = async function () {
        let response1 = await apiCall("customSellRequests", 'GET', '', history)
        // // console.log(response1)
        let responseJSON1 = await response1.json();
        // console.log("Sell Requests", responseJSON1._embedded.customSellRequests)
        setRowUser(responseJSON1._embedded.customSellRequests)
        setLoad(true);
    }
    ...
    // company_name company_id comment sharesInDematForm
            ...
            <div className="dbmn ">
                <div className="Table_title">
                    <div className="d-flex align-items-center justify-content-start w-100">
                        <h5 className='main-heading'><b>Custom Sell Request</b></h5>
                    </div>
                </div>
                                ...
                                rowUser.map(
                                    (user, index) => (
                                        <TableRow key={index} id={user.id}>
                                            ...
                                            <TableCell>{isload ? <> {user.sellQty} </> : <Skeleton width={100} />}</TableCell>
                                            <TableCell>{isload ? <> {user.pricePerShare} </> : <Skeleton width={100} />}</TableCell>
                                            ...
                                            <TableCell>{isload ? <> {user.shareType} </> : <Skeleton width={100} />}</TableCell>
                                            <TableCell>{isload ? <> {user.sharesInDematForm} </> : <Skeleton width={100} />}</TableCell>
                                            <TableCell>{isload ? <> {user.additionalConditions} </> : <Skeleton width={100} />}</TableCell>
                                            {/* <TableCell >*/}
