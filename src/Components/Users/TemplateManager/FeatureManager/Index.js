...
import { apiCall, apiCall1, apiCall12, apiCall25, apiCall26, downloadurl, setAccessToken } from "../../../../Utils/Network"
...
export default function InventoryTableContent(props){
    ...
    const getAllInventoryget = async function (){
        let response1 = await apiCall12("features?projection=CustomFeatureProjection",'GET', '', history)
        //console.log(response1)
        let responseJSON1 = await response1.json();
        //console.log(responseJSON1._embedded.features)
        setRowUser(responseJSON1._embedded.features)

        let response2 = await apiCall("templates/"+props.company.id+"/feature",'GET', '', history)
        //console.log(response2)
        let responseJSON2 = await response2.json();
        // console.log(responseJSON1._embedded.features)
        setselectedFeature(responseJSON2.id)
    }

    const getAllInventoryupdate = async function (id){
        await setselectedFeature(id)

        //console.log(selectedFeature+"hahahaha"+id)

        let response2 = await apiCall26("templates/"+props.company.id+"/feature",'PUT', "features/"
            +id, history)


        if (response2.status === 204) {
            successToast("Success", "successfully linked template with feature!!")
            getAllInventoryget()
                      ...
                      <TableHeader valueToOrderBy={valueToOrderBy} orderDirection={orderDirection} handleRequestSort={handleRequestSort} />
                        {
                            rowUser.map(
                                (user,index) => (
                                  <TableRow  onClick={(e) => getAllInventoryupdate(user.id)} key={index} className={selectedFeature== user.id ? 'ActiveCurrentUser' : null} id={user.id}>
