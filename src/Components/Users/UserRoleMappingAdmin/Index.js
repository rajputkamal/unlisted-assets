...
export default function InventoryTableContent(props) {
  ...
  const getAllInventory = async function () {
    setLoading(true)
    let response1 = await apiCall12("users/search/findByNameContainingOrMobileContaining?searchKey=" + searchkey + "&searchKey1=" + searchkey, 'GET', '', history)
    //console.log(response1)
    let responseJSON1 = await response1.json();
      ...
      <div className="dbmn">
        <div className="Table_title">
          <div className="d-flex align-items-center justify-content-start w-100">
            <h5 className='main-heading'><b>User Role Mapping Admin</b></h5>
          </div>
        </div>
                      ...
                      <div className="form-group">
                        <div className="form-group has-search mb-0 small-icon w-100 ">
                          <div className='inventory-search-icon form-control-feedback'>
                            <SearchIcon onClick={getAllInventory}/>
                          </div>
                          <Autocomplete
                            style={{ width: 1000 }}
                            freeSolo
