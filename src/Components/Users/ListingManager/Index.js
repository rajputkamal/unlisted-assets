...
export default function InventoryTableContent(props) {
    ...
    const getAllInventoryget = async function (pageNum1) {
        ...
        setLoading(true)
        let response1 = await apiCall12("trades/search/findAllTradeSearchAdmin?searchKey=" + searchkey + "&page=" + pageNum1 + "&size=" + pagesize, 'GET', '', history)
        // console.log(response1)
        let responseJSON1 = await response1.json();
        ...
        if (pageNum1 == 0) {
            setRowUser(responseJSON1._embedded.trades)
        } else {
            responseJSON1._embedded.trades.map(record => rowUser.push(record));
            setRowUser([...rowUser])
        }
        ...
        if ((responseJSON1._embedded.trades.length % pagesize) != 0) {
            sethasmore(false)
        } else {
            sethasmore(true)
        }
    ...
    const getAllInventoryupdate = async function (id) {
        let response1 = await apiCall("trades/" + editedData.id, 'PUT', editedData, history)
        // console.log(response1)
        let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id) {
        let response1 = await apiCall("trades/" + editedData.id, 'DELETE', '', history)
        // console.log(response1)
        let responseJSON1 = await response1.json();
        // console.log(responseJSON1)
    ...
    const getAllInventoryadd = async function () {
        let response1 = await apiCall("trades", 'POST', editedData, history)
        // console.log(response1)
        let responseJSON1 = await response1.json();
        // console.log(responseJSON1)
            ...
            <div className="dbmn">
                <div className="Table_title">
                    <div className="d-flex align-items-center justify-content-start w-100">
                        <h5 className='main-heading'><b>Listing Manager</b></h5>
                    </div>
                </div>
                                                    ...
                                                    <Autocomplete
                                                        ...
                                                        renderInput={(params) => (
                                                            <TextField {...params}
                                                                ...
                                                                placeholder="Search Company Name, Account Id"
                                    ...
                                    rowUser.map(
                                        (user, index) => (
                                            <TableRow key={index} id={user.id}>
                                                ...
                                                <TableCell>{isload ? <> {user.qty} </> : <Skeleton width={100} />}</TableCell>
                                                ...
                                                {/* <TableCell>{isload ? <> {user.price} </> : <Skeleton width={100} />}</TableCell>
                                                <TableCell>{isload ? <> {user.minBidPriceAccepted} </> : <Skeleton width={100} />}</TableCell> */}
                                                ...
                                                <TableCell>{isload ? <> {user.tradeStatus} </> : <Skeleton width={100} />}</TableCell> */}
                    ...
                    <div className={classes.paperView}>
                        ...
                        <div className="modal-body">
                            <form className="w-100">
                                ...
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">price -</label>
                                    <h6>{editedData.price ? editedData.price : 'NA'}</h6>
                                </div>
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">minBidPriceAccepted -</label>
                                    <h6>{editedData.minBidPriceAccepted ? editedData.minBidPriceAccepted : 'NA'}</h6>
                                </div>
                                ...
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">tradeType -</label>
                                    <h6>{editedData.tradeType ? editedData.tradeType : 'NA'}</h6>
                                </div>
                                <div className="form-control-row">
                                    <label className="m-0 mt-2 text-small">tradeStatus -</label>
                                    <h6>{editedData.tradeStatus ? editedData.tradeStatus : 'NA'}</h6>
                                </div>
                                    </div>
