...
export default function InventoryTableContent(props) {
    ...
    const getAllInventoryget = async function () {
        let response1 = await apiCall("findBrokers", 'GET', '', history)
        // // console.log(response1)
        let responseJSON1 = await response1.json();
        // // console.log(responseJSON1._embedded.roles)
        setRowUser(responseJSON1._embedded.findBrokers)
        setLoad(true);
    }

    const getAllInventoryupdate = async function (id) {
        let response1 = await apiCall("findBrokers/" + editedData.id, 'PUT', editedData, history)
        // // console.log(response1)
        let responseJSON1 = await response1.json();
    ...
    const getAllInventorydelete = async function (id) {
        let response1 = await apiCall("findBrokers/" + editedData.id, 'DELETE', '', history)
        // // console.log(response1)
        let responseJSON1 = await response1.json();
        // // console.log(responseJSON1)
    ...
    const getAllInventoryadd = async function () {
        let response1 = await apiCall("findBrokers", 'POST', editedData, history)
        // // console.log(response1)
        let responseJSON1 = await response1.json();
        // // console.log(responseJSON1)
            ...
            <div className="dbmn">
                <div className="Table_title">
                    <div className="d-flex align-items-center justify-content-start w-100">
                        <h5 className='main-heading'><b>Broker Shares Service Manager</b></h5>
                    </div>
                </div>
                                ...
                                rowUser.map(
                                    (user, index) => (
                                        <TableRow key={index} id={user.id}>
                                            ...
                                            <TableCell>{isload ? <> {user.qtyHolding} </> : <Skeleton width={100} />}</TableCell>
